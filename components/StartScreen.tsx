...
import { Play, Key, Bot, ChevronRight, CheckCircle, X, ImageOff, Cog, Book } from 'lucide-react';
...
const StartScreen: React.FC<StartScreenProps> = ({ onStart, onStartStoryMode }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col items-center justify-center overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none">
                ...
                {[...Array(20)].map((_, i) => (
                    <motion.div
                        ...
                        style={{ width: Math.random() * 3 + 'px', height: Math.random() * 3 + 'px' } as any}
                    />
                ))}
            </div>
            ...
            >
                ...
                <p className="text-slate-400 font-serif italic max-w-md mb-12 leading-relaxed whitespace-pre-line">
                    {`"침묵하는 신이 되어 역사를 지켜보소서. \n 당신의 한 마디가 경전이 되고, 침묵이 곧 심판이 되나이다."`}
                </p>

                <div className="flex flex-col gap-4 w-full max-w-xs">
                    {/* Standard Mode - The Silent God */}
                    <button
                    ...
                    {/* Story Mode - The Scribe's Penance */}
                    <button
                        onClick={handleStartStory}
                        className="group relative px-8 py-3 bg-purple-900/20 hover:bg-purple-900/30 border border-purple-500/30 rounded-lg transition-all duration-300 overflow-hidden"
                    >
                        ...
                        <div className="flex items-center justify-center gap-3 text-purple-400 font-display font-bold tracking-widest uppercase text-sm">
                            <span>이야기 모드 : 마지막 서기관</span>
                            <Book size={16} />
                        </div>
                    </button>
                    ...
                    <button
                        ...
                        className="flex items-center justify-center gap-2 text-slate-500 hover:text-slate-300 text-xs uppercase tracking-widest py-2 transition-colors mt-2"
                    >
                        <Cog size={12} />
            ...
            <div className="absolute bottom-6 text-[10px] text-slate-600 font-mono">
                v1.1.3.1208 • Created by devroh horved
            </div>
